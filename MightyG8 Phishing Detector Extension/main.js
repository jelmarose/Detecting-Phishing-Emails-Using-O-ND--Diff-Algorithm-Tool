...
function compareEmail(forEmailComp){
	...
	/*Check if forEmailComp is equal to any of the emailValues*/
	while(temp < emailValues.length){
		if(forEmailComp == emailValues[temp]){
			console.log("NO DIFF REQUIRED, PAIR with ", emailValues[temp]);
			diffTotal = 0;
		}
		else{
			temp++;
			if(forEmailComp == emailValues[temp]){
				...
				if(forEmailComp == emailValues[temp]){
					...
					console.log("DIFF REQUIRED.")
					/*Start Diff Algo here*/
						diffMatrix.push("0");
						while(ins < forEmailComp[0].length){
							diffMatrix.push(forEmailComp[0][ins]);
							ins++;
						}
						endOfX = diffMatrix.length-1;
...
function diffMain(filteredMatrix){
	...
	while(i < filteredMatrix[0].length){
		if(filteredMatrix[0][i] == "email"){
			i++;
			resultMatrix.push("id");
			resultMatrix.push(filteredMatrix[0][i-2]);
			resultMatrix.push(filteredMatrix[0][i]);
			forEmailComp.push(filteredMatrix[0][i]);

			var diffValue = compareEmail(forEmailComp);
			console.log("Diff Value of ", filteredMatrix[0][i], " is ", diffValue);
			resultMatrix.push("diffValue");
			resultMatrix.push(diffValue);

			//Get DKIM
			var dkimVal = filteredMatrix[0][i].split("@");
			resultMatrix.push("dkim");
			resultMatrix.push(dkimVal[1]);
		}
...
function evaluation(resultMatrix, fixedInputMatrix){
	...
	*/
	...
	while(i < resultMatrix.length){
		...
		else if(resultMatrix[i] == "links"){
			numOfLink++;
		}
		else if(resultMatrix[i] == "dkim"){
			i++;
			dkimVal = "PASS with domain " + resultMatrix[i];
		}
		i++;
	}

	displayList.push(numOfLink-1);
	displayList.push(dkimVal);
...
function createDisplayMessage(displayList){
	...
	while(i < displayList.length){
		...
		displayMessage = displayMessage + header + dkimResult + linksResult;
	}
